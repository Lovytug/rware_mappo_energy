from environments.base.generate_blocks.generate_blocks import GenerateRandomBlocks
from abc import ABC, abstractmethod
from typing import Any
import random


class FieldExtactor(ABC):
    phase: int
...
class ScalerExtractor(FieldExtactor):
    phase = 10
...
class CoordListExtractor(FieldExtactor):
    phase = 20
    ...
    def generate(self, name: str, cfg: dict, ctx):
        if cfg['mode'] == "fixed":
            ctx.occupied.update([(x, y) for x, y in cfg['coordinates']])
