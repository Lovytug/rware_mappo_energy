...
class AgentGoalRelationBlock(BaseRelationBlock):

    FEATURES_DIM = 10

    relation_type = "agent_goal"

    def __init__(self, cfg: AgentGoalConfig):
        self.cfg = cfg

    def _build_features_vector(self, agent, goal):
        dx = (goal.x - agent.x) / self.cfg.max_width
        dy = (goal.y - agent.y) / self.cfg.max_height
        ...
        return [
            ...
            goal.x/self.cfg.max_width, goal.y/self.cfg.max_height,

            dx, dy, dist
        ]
    
    def _generate_for_goal(self, agent, goals):
        relation = []
        for goal in goals:
            dx = (goal.x - agent.x)
            dy = (goal.y - agent.y)
            ...
            vec = self._build_features_vector(agent, goal)
    ...
    def build(self, repo: EntityRepository):
        ...
        for agent in repo.agents:
            relations = self._generate_for_goal(agent, repo.goals)
